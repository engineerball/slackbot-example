...
class SlackBot(object):
    ...
    def __init__(self, config=None):
        ...
        for name, url in self.config.get('graph_urls').items():
        ...
        if full_text.startswith(self.bot_id):
            ...
            if len(question) > 0:
                ...
                if 'list graph shortcuts' in question:
                    self.respond(channel, 'I know about these graphs: {}'.format(self.graph_shortcuts))
                elif 'graph' in question:
                    self.respond(channel, 'Please wait...', True)
                elif 'help' in question:
                    self.respond(channel, 'I can answer questions about: {}'.format(self.help_msg))
                else:
                    self.respond(channel, 'Sorry, I can only answer questions about: {}'.format(self.help_msg))
    ...
    def generate_and_upload_graph(self, filename, url, channel):
        ...
        container = client.create_container(
            ...
            command='screenshot \'{}\' 1366x768'.format(url),
            volumes=[dir_name],
        ...
        while True:
            ...
            new = []
            for f in files2:
                if f not in files1 and f.endswith(".png"):
                    new.append(f)
            print("new", new)
            for f in new:
